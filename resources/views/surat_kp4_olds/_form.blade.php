...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    ...
    <div>
        <label class="block font-medium mb-1">Disamping Jabatan Utama</label>
        <input type="text" name="disamping_jabatan" value="{{ old('disamping_jabatan', $kp4->disamping_jabatan ?? '') }}" class="w-full border rounded px-3 py-2 dark:bg-gray-700 dark:text-gray-100" placeholder="Bekerja pula sebagai...">
    </div>

    <div>
        <label class="block font-medium mb-1">Penghasilan Disamping</label>
        <input type="text" name="penghasilan_disamping" value="{{ old('penghasilan_disamping', $kp4->penghasilan_disamping ?? '') }}" class="w-full border rounded px-3 py-2 dark:bg-gray-700 dark:text-gray-100" placeholder="Rp - sebulan">
    </div>
        ...
        <table class="min-w-full table-auto border" id="tabel-anggota">
            <thead class="bg-gray-50 dark:bg-gray-700">
                <tr>
                    ...
                    <th class="px-2 py-2 text-left">Penghasilan Sebulan</th>
            ...
            <tbody class="anggota-list">
                @forelse($anggota as $index => $a)
                    @include('surat_kp4_olds._anggota_row', ['a' => $a, 'i' => $index])
                @empty
                    @include('surat_kp4_olds._anggota_row', ['a' => null, 'i' => 0])
                @endforelse
            </tbody>
        </table>
    </div>
